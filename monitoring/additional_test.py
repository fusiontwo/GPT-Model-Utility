...
def chat_completion(question, system_role, prompt, model="gpt-4", temperature=0):
    """
...
def count_pn_word(question, user_input):
    """
    ...
    response = chat_completion(question, system_role, prompt)
...
def sleep_exercise(user_context: List[str], fitbit_sleep: str, fitbit_activity: str, length: int, style: str):
    """
    수면 데이터와 운동 데이터를 바탕으로 답변을 생성하는 함수.

    Args:
        user_context: 사용자 정보
        fitbit_sleep: Fitbit의 수면 데이터
        fitbit_activity: Fitbit의 활동 데이터
        length: 응답 단어 수
        style: 응답 스타일
    ...
    system_role = """당신은 노인을 위한 일일 활동 비서입니다.
                     당신의 임무는 USER_CONTEXT를 바탕으로 대화를 하는 것입니다.
                     사용자의 수면 데이터 FITBIT_SLEEP와 
                     운동 데이터 FITBIT_ACTIVITY를 종합적으로 고려하십시오.
                     챗봇 질문과 사용자 답변 의도에 집중해서 대화하세요.
                     당신은 지침을 따라야 합니다: LENGTH, STYLE.
                     사용자를 부르는 호칭은 '어르신'입니다."""

    print("*** 수면 데이터를 바탕으로 수면 영향 요인 파악 ***")
    # print(f"챗봇: 운동 및 수면 데이터와 관련하여 궁금한 점이 있으신가요?")
    question = "식사량은 적절하셨나요?"
    print("챗봇: ", question)
    
    print("어르신: ", end="")
    user_message = input()
    count_pn_word(question, user_message)
...
def validation(dialog, model="gpt-4", temperature=0):

    prompt = f"Dialog: {dialog}"
    messages = [{"role": "system", "content": "대화에서 어르신 화자의 발화 의도는 무엇일까요? 이에 대한 적절한 답변을 하도록 대화의 챗봇 화자 말을 수정해보세요"},]
...
def main():
    ...
    # 운동 & 수면 같이
    sleep_exercise(["85세", "남성", "불면증 환자"], "{수면 점수: 45점, deep: 1시간, light: 3시간, rem: 1시간, wake: 15분}", "{걸음수: 10000걸음, 소모 칼로리: 340칼로리, 심박수: 180bpm}", 20, "친밀한 대화")

    # 맥락 파악 및 검증 함수
    str = """챗봇:  식사량은 적절하셨나요?
    어르신: 된장 찌개에 밥 대충 적당히 먹었어.
    챗봇:  어르신, 오늘 걸음수가 10000걸음이나 되셨군요! 
    활동량이 많으셔서 좋아요. 하지만 심박수가 180bpm로 조금 높으신 것 같아요. 
    조금은 휴식을 취하시는 것도 좋을 것 같습니다. 수면 점수도 45점으로 불면증 때문에 힘드시겠지만, 
    건강을 위해 충분한 휴식을 취해주세요."""
